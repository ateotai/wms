...
export function ProductLabelModal({ isOpen, onClose, product, autoPrint = false }: ProductLabelModalProps) {
            ...
            <div class="label">
              <div class="row">
                ...
                <div class="info">
                  ...
                  ${product.location ? `<div>Ubicación: ${product.location}</div>` : ''}
                </div>
              </div>
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      try {
        printLabel();
        printedForCodeRef.current = code || null;
      } catch {}
    }, 200);
    return () => clearTimeout(t);
  }, [isOpen, product, autoPrint]);

  return (
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-semibold text-gray-800">Etiqueta de Producto</h3>
        ...
        <div className="border border-gray-200 rounded-md p-3">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <div className="text-sm font-mono">{product.barcode || product.sku}</div>
              <div className="text-xs text-gray-600">{product.name}</div>
              <div className="text-xs text-gray-600">SKU: {product.sku}</div>
              {product.category && <div className="text-xs text-gray-600">Cat: {product.category}</div>}
              {product.location && <div className="text-xs text-gray-600">Ubicación: {product.location}</div>}
            </div>
          </div>
          <div className="mt-2">
