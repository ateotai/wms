...
export function TaskLabelModal({ isOpen, onClose, task, autoPrint = false }: TaskLabelModalProps) {
  ...
  useEffect(() => {
    ...
    const code = task.orderNumber || task.id;
    // Generar QR pequeño
            ...
            <div class="label">
              <div class="row">
                ...
                <div class="info">
                  ...
                  ${task.customer ? `<div>Cliente: ${task.customer}</div>` : ''}
                  ${task.assignedTo ? `<div>Asignado: ${task.assignedTo}</div>` : ''}
                  ${task.location ? `<div>Ubicación: ${task.location}</div>` : ''}
                </div>
              </div>
  ...
  useEffect(() => {
    ...
    // pequeño delay para asegurar canvas listo
    const t = setTimeout(() => {
      try {
        printLabel();
  ...
  }, [isOpen, task, autoPrint]);

  return (
    <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm p-4">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-sm font-semibold text-gray-800">Etiqueta de Tarea</h3>
        ...
        <div className="border border-gray-200 rounded-md p-3">
          <div className="flex items-center">
            ...
            <div className="ml-3">
              ...
              <div className="text-sm font-mono">{task.orderNumber || task.id}</div>
              {task.customer && <div className="text-xs text-gray-600">Cliente: {task.customer}</div>}
              {task.assignedTo && <div className="text-xs text-gray-600">Asignado: {task.assignedTo}</div>}
              {task.location && <div className="text-xs text-gray-600">Ubicación: {task.location}</div>}
            </div>
          </div>
          <div className="mt-2">
